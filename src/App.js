    ...
    <header className="topbar">
      <div className="topbar__inner">
        <div className="brand">
          ...
          <div className="brand__name">PrayerStream</div>
        </div>
...
function LeftSidebar() {
  return (
    <aside className="sidebar sidebar--left">
      ...
      <div className="card">
        <div className="card__header">Trending Requests</div>
        <ul className="widget-list">
          {topNews.map((n) => (
            <li key={n.id}>
              <div className="widget-list__thumb" />
...
function Feed() {
  ...
  const handleNewPost = (text) => {
    ...
    const newPost = {
      ...
      time: "Just now",
      text,
      image: Math.random() > 0.6 ? makePhoto(id) : "",
      prayed: 0,
      comments: 0,
      shares: 0,
    };
...
export default function App() {
  // Example: could gate by auth later
  useMemo(() => {
    document.title = "PrayerStream — Home";
  }, []);
